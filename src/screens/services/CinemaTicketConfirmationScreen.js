...
export default function CinemaTicketConfirmationScreen({ navigation, route }) {
  const {
    reference, booking,
    movieTitle, cinemaName, attendanceDate, showtime, amount,
  } = route.params || {};
      ...
      <ScrollView contentContainerStyle={ss.sc} showsVerticalScrollIndicator={false}>
        ...
        <View style={[ss.statusCard, { backgroundColor: tc.card, borderColor: tc.border || '#E5E5EA' }]}>
          ...
          <Text style={[ss.statusSub, { color: tc.subheading }]}>
            Your cinema ticket has been booked successfully.
          </Text>
        ...
        <View style={[ss.detailCard, { backgroundColor: tc.card, borderColor: tc.border || '#E5E5EA' }]}>
          <View style={ss.cardHeader}>
            <Ionicons name="film-outline" size={16} color={tc.primary} />
            <Text style={[ss.cardTitle, { color: tc.subheading }]}>MOVIE DETAILS</Text>
          </View>
          <InfoRow label="Movie"    value={movieTitle} />
          <InfoRow label="Cinema"   value={cinemaName} />
          <InfoRow label="Date"     value={fmtDate(attendanceDate)} />
          <InfoRow label="Showtime" value={showtime} />
          ...
          <View style={[ss.detailCard, { backgroundColor: tc.card, borderColor: tc.border || '#E5E5EA' }]}>
            ...
            {booking.tickets.map((t, i) => (
              <View key={t.id || i} style={ss.ticketRow}>
                <Text style={[ss.ticketTitle, { color: tc.heading }]}>{t.title || 'Ticket'}</Text>
                {t.price != null && (
