...
export default function DataPurchaseScreen({ navigation, route }) {
  ...
  const validateDataPayment = useCallback((paymentData) => {
    ...
    // Amount is embedded in the plan, so we validate the plan's amount
    if (!paymentData.planAmount || paymentData.planAmount < 50) {
  ...
  const executeDataPurchase = useCallback(async (pin, paymentData) => {
    try {
      const response = await fetch(`${PaymentApiIPAddress}/payments/buy-data`, {
        method: 'POST',
  ...
  const payment = useServicePayment({
    ...
    executePurchase: executeDataPurchase,
    navigation,
    route,
  });
