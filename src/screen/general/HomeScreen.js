...
// Unified services — top 4 are the "quick" ones, rest follow
const SERVICES = [
...
const TX_NAMES = {
  ...
  cac_compliance:    'CAC Compliance Check',
};
...
const TX_ICONS = {
  ...
  flight_booking:    { icon: 'airplane-outline',      gradient: ['#3B82F6', '#6366F1'] },
  ...
  cac_compliance:    { icon: 'shield-checkmark-outline', gradient: ['#0D9488', '#0891B2'] },
};
        ...
        <BlurView intensity={18} tint="light" style={ss.cardBlur}>
          ...
          >
            ...
            <View style={ss.cardBtns}>
              <TouchableOpacity
                style={ss.cardBtnPrimary}
                onPress={() => navigation.navigate('Wallet')}
                activeOpacity={0.85}
              >
                <LinearGradient
                  colors={['#667EEA', '#764BA2']}
                  style={ss.cardBtnGrad}
                  start={{ x: 0, y: 0 }}
                  end={{ x: 1, y: 0 }}
                >
            ...
            <View style={ss.grid}>
              {SERVICES.map(svc => (
                <ServiceCard
                  key={svc.id}
                  svc={svc}
                  onPress={() => svc.screen && navigation.navigate(svc.screen)}
                  labelColor={tc.heading}
                />
              ))}
            </View>
          </View>
            ...
            {transactions.length > 0 ? (
              transactions.slice(0, 5).map((tx, i) => {
                ...
                return (
                  <TouchableOpacity
                    key={tx._id || i}
                    style={[ss.txRow, { borderBottomColor: tc.border, borderBottomWidth: i < transactions.slice(0, 5).length - 1 ? 1 : 0 }]}
                  ...
                  >
                    ...
                    {/* Name + date */}
                    <View style={ss.txMid}>
                      <Text style={[ss.txName, { color: tc.heading }]} numberOfLines={1}>
                        {TX_NAMES[tx.type] || tx.type}
                      </Text>
                    ...
                    <View style={ss.txRight}>
                      <Text style={[ss.txAmount, { color: tc.heading }]}>
